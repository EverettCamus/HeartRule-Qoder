...
async function enableLLMBubbles(page: Page) {
  ...
  const okButton = page.locator('.ant-modal:has-text("调试输出选项") .ant-modal-footer .ant-btn-primary');
...
async function sendMessage(page: Page, message: string, roundNumber: number) {
  ...
  const inputArea = page.locator('textarea[placeholder="Type your message... (Press Enter to send, Shift+Enter for new line)"]');
...
test.describe('调试气泡会话隔离验证', () => {
  ...
  test('验证loadSessionData确实清空了debugBubbles状态', async ({ page }) => {
    // 监听控制台日志来验证清空操作
    const consoleLogs: string[] = [];
    page.on('console', msg => {
      const text = msg.text();
    ...
    const hasClearLog = consoleLogs.some(log => 
      log.includes('Cleared old debug bubbles') || log.includes('🧹')
    );
