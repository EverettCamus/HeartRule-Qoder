...
describe('ScriptExecutor Action State Restoration', () => {
  ...
  it('第1轮：应该创建 action 并序列化状态（currentRound=1）', async () => {
    ...
    const result1 = await executor.executeSession(testScript, 'test-session-1', initialState);
  ...
  it('第2轮：应该恢复 action 状态并继续执行（currentRound 应该从1递增到2）', async () => {
    ...
    const result1 = await executor.executeSession(testScript, 'test-session-2', initialState);
    ...
    // 关键验证：第2轮执行时，action 完成（currentRound 达到 max_rounds）
    // 由于这是最后一个 action，status 应该是 completed
  ...
  it('完整多轮流程：验证 action 实例的连续性', async () => {
    ...
    const result1 = await executor.executeSession(testScript, 'test-session-3', initialState);
    ...
    // 关键验证：
    // 1. 第2轮 action 完成，这是最后一个 action 所以 status 是 completed
    expect(result2.status).toBe('completed');
  ...
  it('BUG重现：如果 action 被重新创建，currentRound 会重置为0', async () => {
    ...
    const result1 = await executor.executeSession(testScript, 'test-session-4', initialState);
    expect(result1.currentAction?.currentRound).toBe(1);
    ...
    // 如果bug修复，action 应该正确完成并返回 completed
