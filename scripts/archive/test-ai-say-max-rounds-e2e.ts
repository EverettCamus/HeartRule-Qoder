...
async function testAiSayMaxRounds() {
  try {
    ...
    const aiMsgs1 = messagesAfterRound1.filter(
      (m) => m.role === 'assistant' && m.actionId === 'say_welcome'
    );
    ...
    const aiMsgs2 = messagesAfterRound2.filter(
      (m) => m.role === 'assistant' && m.actionId === 'say_welcome'
    );
    ...
    const aiMsgs3 = messagesBeforeRound3.filter(
      (m) => m.role === 'assistant' && m.actionId === 'say_welcome'
    );
