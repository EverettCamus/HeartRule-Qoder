...
>
> 一句话：**地图 / 时钟 / 施工队 / 智能通道——四层各归其位，每层有载体、有节奏、有完成态。**
...
| 层                     | 回答的问题                            | 节奏                | 仓库载体                                  | 完成态                                  |
| ---------------------- | ------------------------------------- | ------------------- | ----------------------------------------- | --------------------------------------- | -------------- |
| **地图** DDD 战略设计  | 领域在哪、边界在哪、通用语言是什么    | 月~季度级，证据驱动 | `docs/design/` + `decisions/`             | 文档达到 `decision-recorded` 即封板     |
| **时钟** Scrum（轻量） | 这周做什么、什么时候做完、什么叫 Done | 每周                | `docs/todo/backlog.md` + `sprint-plan.md` | 故事级 DoD 全部勾选                     |
| **施工队** superpowers | 单个故事怎么做                        | 小时~天级           | `docs/superpowers/specs                   | plans/`                                 | 一个可运行增量 |
| **🧠 智能通道**        | AI 引擎实现思路怎么沉淀               | 随时开、产出物驱动  | backlog「智能设计议题」分区               | 收敛为 ADR / spec / backlog 故事 三选一 |

三层以上的定位关系：**地图决定边界，时钟决定节奏，施工队决定方法，智能通道决定引擎灵魂**。四层不得互相污染：

- 设计文档是**地图**，不是 sprint 交付物；需要先写大设计的议题进地图层，不当故事做。
- sprint 故事是**时钟**的交付物，每个故事必须在 1–2 个会话内完成一个可运行增量；做不完说明故事太大或前置设计没定。
- 智能思路讨论是**通道**，产出的是决策/spec/story，不是"聊到哪算哪"。
...
- `docs/todo/backlog.md` — 产品待办。所有候选故事，含三类：`[feature]` / `[design]` / `[intelligence]`。每项：标题、类型、状态（`backlog/ready/in-progress/done`）、关联文档。
- `docs/todo/sprint-plan.md` — 当前 sprint。sprint 目标、选中的故事清单（含 DoD）、状态。每次 sprint 覆盖，旧计划留在 git 历史。
...
| `[intelligence]` | 收敛出 ADR / spec / backlog 故事（三选一）+ commit + push              |

**故事粒度规则**：一个故事应在 1–2 个会话内完成。若某工作"需要先写一篇大设计"，它本质是地图层议题，先走 L2 流程，产出决策后再拆成小故事。

## 4. 地图层：DDD 战略设计

**载体**：`docs/design/*.md` + `docs/design/decisions/*.md`（ADR）。
...
**为什么单列一层**：HeartRule 是 AI 引擎——记忆怎么调取、意识怎么触发、话题队列怎么动态调整，这些"智能实现思路"需要人类与 AI 频繁对话，且天然不适合硬性排期。单列一层让它们有合法位置，不再泄漏。

**规则（产出物驱动）**：智能思路讨论**随时可开**，但每次讨论必须收敛为以下三选一：

1. **ADR** —— 决策已定，记录进 `docs/design/decisions/`；
2. **spec** —— 思路已成型，写进 `docs/superpowers/specs/`；
3. **backlog 故事** —— 思路还需实施，沉淀为 `[intelligence]` 故事进 backlog。
...
1. **先看 `docs/todo/sprint-plan.md`** —— 知道当前 sprint 做什么、哪些故事 in-progress。
2. **若涉及设计**，查 `docs/design/README.md` 确认相关文档状态，遵守封板规则。
3. **若涉及智能思路**，按第 6 节收敛规则执行，产出物必须落地。
