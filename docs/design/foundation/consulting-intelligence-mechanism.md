...
# 咨询智能实现机制：五层递归的规划-执行架构

> **文档定位**：设计哲学之后的整体思路——"抬头规划、低头执行"如何落地为一套递归的智能架构。本文回答"智能放在哪里、各层怎么规划怎么执行、层间如何通信"，介于设计哲学（为什么）与具体机制文档（怎么做）之间。
>
> **关联文档**：
>
> - 上游：[HeartRule 设计哲学 v2](heartrule-design-philosophy-v2.md) — 第一性原理与八大设计原则（本文是其"总体架构"展开）
> - 下游：[话题单元建模](../topic/topic-unit-modeling.md) — Topic 层的领域模型；[意识系统设计](../consciousness/consciousness-system.md) — 层间调整的裁决机制；[Action 层与 Topic 层职能边界](../topic/action-topic-boundary.md) — 指令包与结果反馈契约
> - 约束：[架构约束清单](architecture-constraints.md) — 红线
>
> **版本**：v0.1.0（草案，复活自归档）
> **创建**：2026-02-10（原文）· 2026-09-13（复活复制）

---

## 演进注记（复活时添加，非原文内容）

1. **规划权收敛**：原文五层各有"大脑"（灵活面），均可动态调整下层计划。按现行意识系统设计的不变量——**意识为唯一队列修改者**——各层的灵活面应读作**调整建议者**，队列的实际修改由意识层裁决执行。本文描述的是"调整能力分层部署"，裁决权归属以意识系统文档为准。
2. **Consultation 层（疗程蓝图）属远景**：现行引擎止于单次会话（Session），跨会话疗程蓝图尚未实现；跨会话状态权威归信息点文档（ADR 007）。原文 Consultation 层内容作为远景蓝图读，非近期实现目标。
3. **示例语法以现行 DSL 为准**：原文落地示例含 `ai_choice` / `ai_form` / `show` / `action_type` 等旧语法，原文已声明"yaml格式仅示例，非最终语法"。现行动作类型为 `ai_say` / `ai_ask` / `ai_think`，具体语法见[脚本引擎设计原则](script-engine-design-principles.md)。
4. **与哲学 v2 §4.1 的"五层"是两回事**：哲学 v2 §4.1 的五层是**技术栈分层**（表现/应用/引擎/脚本/基础设施）；本文五层是**智能递归分层**（疗程/会话/阶段/话题/动作）。互为补充、不冲突，阅读时勿混淆。
5. **来源**：复活自 `docs-archive/misc/HeartRule咨询智能实现机制.md`（2026-03-13 归档，原始路径 docs/design/thinking/）。2026-09-11 经人确认作为"哲学之后的整体思路"复活。
...
topic:
  ...
  # 计划面：一个最简化的默认行动模板（示意本 Topic 在会谈脚本中的 `actions` 列表片段）
  actions:
