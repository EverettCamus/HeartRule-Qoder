...
- 抽象层可先走规则，命中率不理想时升级为极短 LLM 摘要（与 `ai-ask-memory-recall.md` §3.3 的既有机制一致）
- 判断"是否事件性描述"是零成本的启发式规则

**降级策略**（轴 9 `strict_200ms` 预算约束）：
...
| 检索视角来源                     | 归属                       | 理由                                                 |
| -------------------------------- | -------------------------- | ---------------------------------------------------- |
| 普适方向（类似/关联事件）        | 引擎默认检索增强（§2.11）  | 跨领域普适，零配置，≤200ms 满足主线即时理解          |
