...
# Action 层与 Topic 层职能边界：指令包与结果反馈契约

> **文档定位**：定义 Action 层（ai_ask 原子执行）与 Topic 层（战术规划）之间的职责边界与协作契约——指令包下发、结果反馈上报、谁有权调整队列。本文是「意识层与主线引擎的接缝协议」议题（backlog 智能设计议题）的输入材料。
>
> **关联文档**：
>
> - 上游：[话题单元建模](topic-unit-modeling.md) — 话题队列的领域模型
> - 同级：[议程实现机制](topic-queue-implementation.md) — 队列运行时实现
> - 下游：[意识系统设计](../consciousness/consciousness-system.md) — 意识层对话题队列的调整机制
> - 约束：[脚本引擎设计原则](../foundation/script-engine-design-principles.md) — 规划-执行分离的 DSL 表达
>
> **版本**：v0.1.0（草案，复活自归档）
> **创建**：2026-03（原文）· 2026-09-10（复活复制）

---

## 演进注记（复活时添加，非原文内容）

1. **队列调整权归属已变**：原文将队列调整权（增删改跳）赋予 Topic 层。按现行意识系统设计的不变量——**意识为唯一队列修改者**——本文中 Topic 层的「条件分支与流程调整」职责应读作**意识层职责的历史表述**：Topic 层产生调整建议，队列的实际修改由意识层裁决执行。这正是接缝协议议题要落定的契约。
2. **术语映射**：文中 `ai_talk` / `ai_answer` 为旧动作名，现行动作类型为 `ai_say` / `ai_ask` / `ai_think`；文中 `stage` 对应现行 `phase`。
3. **来源**：复活自 `docs-archive/misc/Action层(ai_ask)与Topic层的职能边界.md`（2026-03-13 归档，原始路径 docs/design/thinking/）。2026-09-10 docs-archive 引擎机制盘点中识别为"未开发且有参考价值"。
...
1. **动作序列的生成与编排**
   - **职责**：将 Topic 下预置的 `actions` 默认模板（如“收集一位抚养者信息”的那组提问动作）**实例化并编排**成具体的、顺序执行的 `Action` 队列。
