...
| backlog     | [feature]      | recall 取回接线与实证：① 实体态进**人物卡**——每轮都取（端口补 `maxEntityTokens`）、按 `canonical_name` upsert 成卡（会话内保留、上限 10 张、按末次提及淘汰），**渲染只在"这一轮提到这个人"时**（判定字段写了专名，或字面扫到卡里已有的名字），**不额外发检索**；② 开 `preferObservations`（概要优先于原始片段，`sourceFactIds` 留展开路径）；③ 实证：写时实体质量与 observations 颗粒度、开关对"事"类召回的影响（需 docker 后端） | [ai_ask 记忆调用 §4.2 / 附 C.1 / 附 C.6](../design/memory/ai-ask-memory-recall.md) · `hindsight-adapter.ts`                                          |
