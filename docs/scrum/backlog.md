...
| backlog     | [feature]      | recall 取回接线与实证：① 实体态进**人物卡**——每轮都取（端口补 `maxEntityTokens`）、按 `canonical_name` upsert 成卡（会话内保留、上限 10 张、按末次提及淘汰），**渲染只在"这一轮提到这个人"时**（判定字段写了专名，或字面扫到卡里已有的名字），**不额外发检索**；② 开 `preferObservations`（概要优先于原始片段，`sourceFactIds` 留展开路径）；③ 实证：写时实体质量与 observations 颗粒度、开关对"事"类召回的影响（需 docker 后端） | [ai_ask 记忆调用（继任稿）§4.2 / 附 C.1 / 附 C.6](../design/memory/ai-ask-memory-recall-rewrite-v2-draft.md) · `hindsight-adapter.ts`                |
...
| 连发拼接还必要吗           | 先拼成一条的理由（「否则指代落在后几条会被漏掉」）是规则路径的说法；判定并入生成后还必要吗、留给谁（判定行的输入还是固定拼法） | [重写稿](../design/memory/ai-ask-memory-recall-rewrite-draft.md) §4「约束各归各家」；2026-09-15 挂，实现时收                                                                                        |
| 反问轮走哪个 move          | 默认模板的 `话术调整策略` 七行里没有"用户反问"这一行——用户直接要方案时，框架该给建议、该先澄清，还是该把它当材料谈？           | [重写稿](../design/memory/ai-ask-memory-recall-rewrite-draft.md) §3.2；记忆侧已定"判据只看缺不缺窗口外事实"，move 本身归模板侧（Default 模板 `config/prompt-defaults/ai_ask_v1.md`）；2026-09-16 挂 |
