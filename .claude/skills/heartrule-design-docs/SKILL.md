...
**核心原则**：文档必须封板——关键决策落 ADR 后 `active` → `decision-recorded`，**停止积累**；修订仅证据驱动。节奏依据：`docs/process/development-rhythm.md` §6。
...
2. **新文档登记**索引：设计类文档登记 `docs/design/README.md`（列：`| 文档 | 层 | 状态 | 已记录决策 |`；层值：基础哲学 / 地图·领域建模 / 本体 / 引擎机制 / 横切约束）；过程类文档登记 `docs/process/README.md`（活文档，`active` 为常态，不封板，按 §10 演进）。
3. **封板**：关键决策落 ADR 后 `active` → `decision-recorded`。封板后**停止积累**——新想法另起 ADR / 新文档 / backlog 探索项；修订仅**证据驱动**（实现碰壁、e2e 暴露问题、领域理解深化），改完重新封板。
4. **ADR 流程**：AI 起草（模板见 `references/adr-template.md`）→ **人批准** → 落 `docs/design/decisions/NNN-slug.md`（编号取现有最大 +1）→ 更新 README「已记录决策」列 → 更新架构约束清单（`docs/design/architecture-constraints.md`——红线增删需人确认）。
