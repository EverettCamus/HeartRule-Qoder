...
- 变量提取与记忆/知识管理：在对话中抽取关键变量，维护多层作用域变量，并与长期记忆和知识结构协同
...
- 核心域（Core Domain）
  ...
  - 能力：脚本解析与执行、变量管理与记忆/知识接口协作、多轮 AI 交互
...
2. 领域规则与应用编排的混杂
   ...
   - LLM 调用细节在 Action 执行器与引擎之间的分工需要进一步澄清

3. 跨上下文边界相对模糊
   - 会话执行 BC 与脚本管理 BC 在数据库与 API 层交界处存在一定耦合
   - 会话执行 BC 与记忆与知识 BC 的职责在领域模型、引擎与共享类型中交织，尤其在“变量控制状态”与“长期记忆/知识沉淀”的分工上需要更明确的边界约定
