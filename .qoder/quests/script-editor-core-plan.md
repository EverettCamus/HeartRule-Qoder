...
在调试配置区增加一个「调试输出过滤器」按钮（🔧图标），点击后弹出多选面板：
...
**过滤器状态持久化**：
- 用户的选择保存到 localStorage
- 下次打开调试面板时自动应用上次的设置
- 默认开启：错误信息、变量状态
- 默认关闭：LLM提示词、执行日志（减少干扰）

#### 2.4.2 调试信息气泡设计

每种类型的调试信息都以气泡形式嵌入到对话流中，使用不同颜色和图标区分：
...
```
📝 [执行日志] - Action: ai_ask_name
开始时间: 2026-01-18 16:45:30
结束时间: 2026-01-18 16:45:31
总耗时: 1.23s
执行状态: 成功 ✅
...
**单个气泡操作**：
- 点击「展开」按钮：展开当前气泡，显示详细信息
- 点击「折叠」按钮：收起当前气泡，仅显示摘要
- 默认状态：所有气泡初始为折叠状态

**批量操作**（通过过滤器面板）：
- 「全部展开」：展开当前显示的所有调试信息气泡
- 「全部折叠」：折叠当前显示的所有调试信息气泡
- 「仅显示错误」：仅显示错误信息气泡，隐藏其他类型
- 「显示全部」：显示所有被选中类型的气泡

**气泡状态记忆**：
- 用户手动展开/折叠的状态在当前会话中保持
- 新生成的气泡默认为折叠状态
- 错误信息气泡默认自动展开（引起注意）

#### 2.4.4 错误详情弹窗（保留但优先级降低）

由于错误信息已经在气泡中展开，详情弹窗可作为可选增强功能：

点击错误气泡中的「查看详情」按钮时，弹出更大的模态窗口：
...
│                                 │
│ 堆栈跟踪（可展开）:             │
│   at ScriptExecutor.execute     │
│   at SessionManager.process     │
│   ...                           │
│                                 │
│          [复制详情] [关闭]       │
└─────────────────────────────────┘
...
// LLM 提示词气泡内容
interface LLMPromptBubbleContent {
...
// 执行日志气泡内容
interface ExecutionLogBubbleContent {
...
interface DebugOutputFilter {
  ...
  expandAll: boolean;            // 全部展开
  errorOnly: boolean;            // 仅显示错误
}
...
interface DebugPanelState {
  ...
  userMessages: Message[];       // 用户消息
  aiMessages: Message[];         // AI 消息
}
```

**状态管理逻辑**：
1. 每次 Action 执行后，根据过滤器配置生成相应的气泡
2. 错误信息气泡自动展开，其他类型默认折叠
3. 气泡按时间顺序插入到对话流中
4. 用户可以手动展开/折叠任意气泡
5. 过滤器设置保存到 localStorage，下次打开时恢复
...
#### 2.7.1 错误信息气泡验收
- ✅ 人为构造脚本语法错误（如删除必填字段），错误气泡以红色背景显示在对话流中
- ✅ 错误气泡默认处于展开状态，显示错误类型、描述、执行位置
- ✅ 点击「折叠」按钮后，错误气泡收起为摘要形式
- ✅ 点击「展开详情」按钮后，显示完整的技术详情、修复建议
- ✅ 点击「重新开始」按钮能成功创建新会话并清空错误状态
- ✅ 模拟 LLM 服务超时错误，错误信息能正确展示执行位置（Action 路径）

#### 2.7.2 LLM 提示词气泡验收
- ✅ 当过滤器中开启「LLM 提示词」选项时，每次 Action 执行后显示蓝色提示词气泡
- ✅ 折叠状态显示提示词的前 50 个字符预览
- ✅ 点击「展开全文」后，显示完整的系统提示词、用户提示词和对话历史
- ✅ 点击「复制提示词」按钮能复制完整内容到剪贴板

#### 2.7.3 LLM 响应气泡验收
- ✅ 当过滤器中开启「LLM 响应」选项时，显示深蓝色响应气泡
- ✅ 展开状态显示模型名称、Token 数、原始响应和处理后响应
- ✅ 能区分原始响应和展示给用户的处理后响应

#### 2.7.4 变量状态气泡验收
- ✅ 当过滤器中开启「变量状态」选项时，显示绿色变量气泡
- ✅ 折叠状态显示新增的变量名称和值
- ✅ 展开状态按 Session/Phase/Topic 级别分组显示所有变量
- ✅ 点击「导出JSON」按钮能下载当前变量状态的 JSON 文件

#### 2.7.5 执行日志气泡验收
- ✅ 当过滤器中开启「执行日志」选项时，显示灰色日志气泡
- ✅ 折叠状态显示执行状态（成功/失败）和耗时
- ✅ 展开状态显示详细的执行步骤和每步耗时

#### 2.7.6 位置信息气泡验收
- ✅ 当过滤器中开启「位置信息」选项时，显示黄色位置气泡
- ✅ 展开状态显示 Phase/Topic/Action 的索引、ID 和名称
- ✅ 点击「在导航树中定位」按钮能跳转到导航树并高亮对应节点

#### 2.7.7 过滤器功能验收
- ✅ 点击「调试输出过滤器」按钮弹出配置面板
- ✅ 勾选/取消勾选任意选项后，对话流中的气泡类型相应显示/隐藏
- ✅ 点击「全部展开」后，所有可见气泡自动展开
- ✅ 点击「全部折叠」后，所有可见气泡自动折叠
- ✅ 点击「仅显示错误」后，仅显示错误信息气泡
- ✅ 过滤器配置保存到 localStorage，关闭再打开后配置保持
- ✅ 点击「重置默认」按钮后，恢复为默认配置（仅显示错误和变量）
