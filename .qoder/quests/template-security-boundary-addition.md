...
| 任务 ID | 任务描述                                             | 估算工作量 | 依赖    |
| ------- | ---------------------------------------------------- | ---------- | ------- |
| T1      | 编写安全边界通用模板片段                             | 2h         | 无      |
| T2      | 更新 ai_ask 现有模板（增加安全边界段落）             | 1h         | T1      |
| T3      | 更新 ai_say 现有模板（增加安全边界段落）             | 1h         | T1      |
| T4      | **框架地基：实现 TemplateResolver 基础类**           | 4h         | 无      |
| T5      | **框架地基：实现四层模板路径解析逻辑**               | 5h         | T4      |
| T6      | 实现 BaseAction.checkSafetyBoundary()                | 3h         | 无      |
| T7      | 更新 AiAskAction 和 AiSayAction（集成多层模板解析）  | 3h         | T5      |
| T8      | 实现模板验证机制（TemplateManager.validateTemplate） | 2h         | T5      |
| **T9**  | **实现 ProjectInitializer 工程初始化服务**           | **5h**     | **T5**  |
| **T10** | **实现 SystemTemplateManager 系统模板管理服务**      | **3h**     | **T9**  |
| **T11** | **开发编辑器创建工程 UI 和 API**                     | **4h**     | **T10** |
| **T12** | **制作工程模板库（CBT评估/CBT咨询/DBT等）**          | **4h**     | **T10** |
| T13     | 编写单元测试（安全边界检测）                         | 2h         | T6      |
| T14     | 编写单元测试（多层模板解析逻辑）                     | 3h         | T5      |
| T15     | 编写集成测试（自定义模板覆盖）                       | 2h         | T7      |
| T16     | 编写集成测试（四层解析回退链）                       | 2h         | T7      |
| **T17** | **编写集成测试（工程初始化流程）**                   | **2h**     | **T11** |
| T18     | 更新项目文档（多层模板系统使用指南）                 | 3h         | 所有    |

**总工作量估算**：49 小时（约 6.5 个工作日）

**新增任务说明**：

- T9-T12 为工程初始化机制相关任务，确保编辑器能自动创建规范化工程结构
- ProjectInitializer 负责工程目录和文件初始化逻辑
- SystemTemplateManager 负责系统模板的分发和版本管理
- 工程模板库包含多个预设场景（CBT评估、CBT咨询、DBT等），降低用户上手成本

**总工作量估算**：33 小时（约 4.5 个工作日）

**重点说明**：

- T4-T5 为框架地基建设任务，确保多层模板解析架构可扩展
- TemplateResolver 设计为独立模块，不与 BaseAction 耦合
- T9-T12 为工程初始化机制任务，确保编辑器能自动创建带有完整模板框架的工程
- 支持未来新增层级（如语言层）时无需大规模重构
...
- **优先级 P0**，必须最先完成
- 构建 TemplateResolver 多层解析框架
- 确保架构可扩展，为未来演进奠定基础
- 完成后进行架构审查

**阶段 2：安全边界系统（T1-T3, T6）**
...
- 实现工程初始化服务和系统模板管理
- 开发编辑器创建工程 UI 和 API
- 制作多个领域工程模板（CBT评估、CBT咨询、DBT等）
- 确保用户创建工程时自动获得完整的模板框架
...
| 验收项                   | 标准                                                                           |
| ------------------------ | ------------------------------------------------------------------------------ |
| 1. 安全边界完整性        | 所有 Action 默认模板包含【安全边界】段落，覆盖 7 类通用约束                    |
| 2. 多层模板解析          | 支持四层模板路径解析（系统默认、领域通用、领域场景、脚本覆盖），回退链正确     |
| 3. 框架可扩展性          | TemplateResolver 设计为独立模块，支持未来新增层级无需大规模重构                |
| 4. 领域和场景配置        | 脚本可声明 domain/scenario，系统能正确加载对应层级的模板                       |
| 5. 脚本覆盖机制          | 支持 config.override_template 配置，能正确加载自定义模板                       |
| 6. 模板验证              | 加载模板时验证变量完整性、安全边界声明，验证失败时有明确警告                   |
| 7. 调试信息              | metadata 中包含 template_path, template_layer, fallback_chain 等详细信息       |
| 8. 测试覆盖率            | 新增代码单元测试覆盖率 >85%，所有集成测试通过                                  |
| 9. 向后兼容性            | 现有 ai_ask / ai_say 脚本无需修改即可运行（自动使用系统默认层）                |
| 10. 编辑器支持           | 编辑器提供领域/场景模板管理功能，支持可视化创建和编辑                          |
| **11. 工程初始化完整性** | **创建新工程时自动初始化四层模板目录结构，复制系统默认模板**                   |
| **12. 工程模板库**       | **提供多个预设工程模板（空白/CBT评估/CBT咨询/DBT等），包含领域模板和示例脚本** |
| **13. 系统模板版本管理** | **编辑器启动时检查系统模板版本，有更新时提示用户同步**                         |
| **14. 初始化性能**       | **从点击"创建工程"到完成初始化，耗时 <3 秒**                                   |
| 15. 文档完整性           | 提供多层模板体系说明、安全边界规范、领域工程师使用指南、多语言工程化方案       |
