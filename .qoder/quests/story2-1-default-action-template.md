  ...
  test('生成基础TopicPlan(直接返回模板)', async () => {
    ...
    const context: TopicPlanningContext = {
      topicConfig: {
        topic_id: 'test_topic',
        actions: [
          { action_id: 'action_1', action_type: 'ai_say', config: { content: 'Hello' } },
        ],
        strategy: '测试策略',
      },
...
describe('ScriptExecutor Topic Planning Integration', () => {
  ...
  test('从实例化队列读取Actions执行', async () => {
    ...
    executionState.currentTopicPlan = {
      ...
      plannedAt: new Date().toISOString(),
      instantiatedActions: [
        { action_id: 'instantiated_action', action_type: 'ai_say', config: { content: 'From Plan' } },
      ],
    };
...
describe('Topic Schema Validation with Strategy', () => {
  test('验证包含strategy的Topic配置', () => {
    ...
    const topicData = {
      ...
      strategy: '这是策略描述',
      actions: [
        { action_id: 'a1', action_type: 'ai_say', config: { content: 'Hello' } },
      ],
    };
...
- [ ] Topic配置支持可选的`strategy`字段,Schema验证通过
- [ ] Topic下的`actions`列表在语义上被视为"默认Action模板"
- [ ] ScriptExecutor进入Topic时调用TopicPlanner生成实例化队列
- [ ] 实例化队列存储在ExecutionState.currentTopicPlan中
- [ ] 未提供TopicPlanner时,自动使用BasicTopicPlanner(向后兼容)
- [ ] BasicTopicPlanner正确返回模板actions的深拷贝
- [ ] 占位符格式验证正确识别合法和非法格式
- [ ] 输入占位符引用未定义变量时生成Warning(不阻断)
- [ ] ExecutionState序列化和反序列化包含currentTopicPlan

### 性能验收

- [ ] BasicTopicPlanner.plan()执行时间 < 10ms (无LLM调用)
- [ ] 包含TopicPlan的ExecutionState序列化大小增量 < 5KB
- [ ] 现有脚本执行性能无明显下降(允许±5%波动)

### 兼容性验收

- [ ] 现有脚本(无strategy字段)无需修改即可正常运行
- [ ] 现有单元测试和集成测试全部通过
- [ ] SessionApplicationService接口签名未变化
- [ ] 数据库Migration无需额外操作(ExecutionState结构兼容)

### 代码质量验收

- [ ] 新增代码通过TypeScript编译,无类型错误
- [ ] 新增代码通过ESLint检查,无警告
- [ ] 单元测试覆盖率 ≥ 80%
- [ ] 集成测试覆盖核心流程(进入Topic→规划→执行)
- [ ] 代码遵循项目DDD架构规范,文件放置正确

### 文档验收

- [ ] 更新脚本编写指南,说明strategy字段用法和最佳实践
- [ ] 更新API文档,说明ExecutionState.currentTopicPlan结构
- [ ] 提供占位符使用示例(输入/输出/Action ID三种类型)
- [ ] 记录关键设计决策和理由
