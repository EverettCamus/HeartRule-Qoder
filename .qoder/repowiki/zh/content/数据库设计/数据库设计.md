...
- [packages/api-server/drizzle/meta/0002_snapshot.json](file://packages/api-server/drizzle/meta/0002_snapshot.json)
- [packages/api-server/drizzle/meta/0003_snapshot.json](file://packages/api-server/drizzle/meta/0003_snapshot.json)
...
- [packages/api-server/drizzle/0002_familiar_joystick.sql](file://packages/api-server/drizzle/0002_familiar_joystick.sql)
- [packages/api-server/drizzle/0003_add_deprecated_status.sql](file://packages/api-server/drizzle/0003_add_deprecated_status.sql)
- [packages/api-server/drizzle/0003_smiling_doctor_strange.sql](file://packages/api-server/drizzle/0003_smiling_doctor_strange.sql)
...
- [docs/MIGRATION_TO_DATABASE_ARCHITECTURE.md](file://docs/MIGRATION_TO_DATABASE_ARCHITECTURE.md)
</cite>
...
**更新** 本版本反映了数据库架构的重大迁移，从混合模式升级到纯数据库模式，引入了新的模板文件类型、虚拟路径系统和弃用状态机制。
...
- [packages/api-server/src/db/schema.ts](file://packages/api-server/src/db/schema.ts#L1-L300)
...
- [packages/api-server/src/routes/projects.ts](file://packages/api-server/src/routes/projects.ts#L1-L1070)
- [packages/api-server/src/routes/versions.ts](file://packages/api-server/src/routes/versions.ts#L1-L554)
...
- [packages/api-server/drizzle/meta/0002_snapshot.json](file://packages/api-server/drizzle/meta/0002_snapshot.json#L1-L1007)
- [packages/api-server/drizzle/meta/0003_snapshot.json](file://packages/api-server/drizzle/meta/0003_snapshot.json#L1-L1007)
...
- [packages/api-server/drizzle/0002_familiar_joystick.sql](file://packages/api-server/drizzle/0002_familiar_joystick.sql#L1-L100)
- [packages/api-server/drizzle/0003_add_deprecated_status.sql](file://packages/api-server/drizzle/0003_add_deprecated_status.sql#L1-L25)
- [packages/api-server/drizzle/0003_smiling_doctor_strange.sql](file://packages/api-server/drizzle/0003_smiling_doctor_strange.sql#L1-L4)
- [packages/api-server/src/db/index.ts](file://packages/api-server/src/db/index.ts#L1-L26)
- [packages/api-server/src/db/migrate.ts](file://packages/api-server/src/db/migrate.ts#L1-L23)
- [packages/api-server/drizzle.config.ts](file://packages/api-server/drizzle.config.ts#L1-L16)
- [scripts/init-db/01-extensions.sql](file://scripts/init-db/01-extensions.sql#L1-L17)
...
- 会话表（sessions）：承载一次咨询会话的生命周期与运行态，包含位置指针、变量快照、元数据与时间戳，新增版本绑定字段。
- 消息表（messages）：记录会话内的用户/助手/系统消息，支持按时间排序与会话维度检索。
- 脚本表（scripts）：存储 YAML 脚本与解析后 JSON 结构，支持版本、状态与标签管理。
- 项目表（projects）：工程主表，关联文件、草稿与版本；支持状态、作者、标签与元数据，新增弃用状态。
- 文件表（script_files）：工程内各类文件（全局、角色、技能、表单、规则、会话、模板）的 JSON/YAML 存储，新增虚拟路径支持。
...
**更新** 新增了模板文件类型（fileType='template'）和虚拟路径系统，支持系统模板的数据库化存储。

**章节来源**

- [packages/api-server/src/db/schema.ts](file://packages/api-server/src/db/schema.ts#L8-L41)
- [packages/api-server/src/db/schema.ts](file://packages/api-server/src/db/schema.ts#L46-L74)
- [packages/api-server/src/db/schema.ts](file://packages/api-server/src/db/schema.ts#L79-L98)
- [packages/api-server/src/db/schema.ts](file://packages/api-server/src/db/schema.ts#L103-L125)
- [packages/api-server/src/db/schema.ts](file://packages/api-server/src/db/schema.ts#L130-L153)
- [packages/api-server/src/db/schema.ts](file://packages/api-server/src/db/schema.ts#L155-L187)
- [packages/api-server/src/db/schema.ts](file://packages/api-server/src/db/schema.ts#L192-L201)
- [packages/api-server/src/db/schema.ts](file://packages/api-server/src/db/schema.ts#L206-L227)
- [packages/api-server/src/db/schema.ts](file://packages/api-server/src/db/schema.ts#L232-L253)
- [packages/api-server/src/db/schema.ts](file://packages/api-server/src/db/schema.ts#L258-L279)
...
- [packages/api-server/src/db/schema.ts](file://packages/api-server/src/db/schema.ts#L46-L227)
...
- 字段要点
  ...
  - 变量快照：JSONB 存储会话变量
  - 元数据：JSONB 扩展字段
  - 版本绑定：新增 version_id 和 version_snapshot 字段，支持版本化会话
  - 时间戳：创建/更新/完成时间
- 索引策略
  - user_id、status、created_at、version_id 四类常用过滤字段建立索引
- 使用场景
  - 会话创建、状态推进、变量快照读写、位置指针更新、版本化会话管理

**更新** 新增了版本绑定字段，支持会话与工程版本的关联。

**章节来源**

- [packages/api-server/src/db/schema.ts](file://packages/api-server/src/db/schema.ts#L46-L74)
- [packages/api-server/drizzle/meta/0003_snapshot.json](file://packages/api-server/drizzle/meta/0003_snapshot.json#L683-L800)
...
- [packages/api-server/src/db/schema.ts](file://packages/api-server/src/db/schema.ts#L79-L98)
- [packages/api-server/drizzle/meta/0003_snapshot.json](file://packages/api-server/drizzle/meta/0003_snapshot.json#L112-L195)
...
- [packages/api-server/src/db/schema.ts](file://packages/api-server/src/db/schema.ts#L103-L125)
- [packages/api-server/drizzle/meta/0003_snapshot.json](file://packages/api-server/drizzle/meta/0003_snapshot.json#L557-L682)
...
- 项目表（projects）
  ...
  - 状态：新增 deprecated 弃用状态，支持软删除
  - 作者/标签/元数据
  - 时间戳
- 工程文件（script_files）
  - 文件类型枚举：新增 template 类型，支持系统模板存储
  - 文件名/内容（JSON/YAML）
  - 虚拟路径：新增 file_path 字段，支持模板层级和方案标识
- 工程草稿（project_drafts）
  - 草稿文件快照、校验状态与错误、更新人
...
- 索引策略
  ...
  - 版本：项目/发布时间索引
- 使用场景
  - 工程创建/复制/归档、文件 CRUD、草稿与版本管理、回滚、模板管理

**更新** 项目表新增了 deprecated 弃用状态，文件表新增了虚拟路径支持和模板文件类型。

**章节来源**

- [packages/api-server/src/db/schema.ts](file://packages/api-server/src/db/schema.ts#L130-L227)
- [packages/api-server/drizzle/meta/0003_snapshot.json](file://packages/api-server/drizzle/meta/0003_snapshot.json#L356-L569)
- [packages/api-server/src/routes/projects.ts](file://packages/api-server/src/routes/projects.ts#L35-L1070)
...
- [packages/api-server/src/db/schema.ts](file://packages/api-server/src/db/schema.ts#L232-L253)
- [packages/api-server/drizzle/meta/0003_snapshot.json](file://packages/api-server/drizzle/meta/0003_snapshot.json#L1-L111)
...
- 工程状态：draft/published/archived/deprecated
- 文件类型：global/roles/skills/forms/rules/session/template
- 校验状态：valid/invalid/unknown

**更新** 工程状态新增 deprecated 弃用状态，文件类型新增 template 模板类型。

**章节来源**

- [packages/api-server/src/db/schema.ts](file://packages/api-server/src/db/schema.ts#L8-L41)
- [packages/api-server/drizzle/meta/0003_snapshot.json](file://packages/api-server/drizzle/meta/0003_snapshot.json#L4-L111)
...
- 快照与 SQL
  ...
  - 0002_familiar_joystick.sql 引入了新的文件表结构
  - 0003_add_deprecated_status.sql 和 0003_smiling_doctor_strange.sql 引入了弃用状态和模板文件类型
  - 0000_snapshot.json 与 0001_snapshot.json 与 0002_snapshot.json 与 0003_snapshot.json 记录各版本快照，便于回溯与对比
- 扩展
...
GEN --> SQL2["0002_familiar_joystick.sql"]
GEN --> SQL3["0003_add_deprecated_status.sql"]
GEN --> SQL4["0003_smiling_doctor_strange.sql"]
SQL0 --> SNAP0["0000_snapshot.json"]
SQL1 --> SNAP1["0001_snapshot.json"]
SQL2 --> SNAP2["0002_snapshot.json"]
SQL3 --> SNAP3["0003_snapshot.json"]
SQL4 --> SNAP3
...
- 索引策略
  - 会话：按 user_id、status、created_at、version_id 建立索引，支持用户会话列表、状态筛选和版本化查询
  - 消息：按 session_id 与 timestamp 建立索引，满足历史消息分页与排序
  - 脚本：按 script_type+status 与 script_name 建立索引，提升脚本检索与去重
  - 工程/文件/版本：按 status/author/name、project_id、file_type、file_path、published_at 建立索引，优化工程与版本管理查询
- JSONB 查询
...
- 模板文件优化
  - 新增的虚拟路径索引支持高效的模板文件检索和层级管理

**更新** 新增了针对模板文件的虚拟路径索引，优化模板文件的查询性能。
...
- 数据不一致
  - 通过 0000_snapshot.json 与 0001_snapshot.json 与 0002_snapshot.json 与 0003_snapshot.json 对比版本差异，定位变更点
- 版本回滚
  - versions 路由支持回滚到指定版本，回滚后会创建新版本记录并标记 is_rollback
- 模板文件问题
  - 检查 script_files 表中的 file_type 是否包含 'template' 值
  - 验证 file_path 字段是否正确设置虚拟路径
...
- [packages/api-server/drizzle/meta/0002_snapshot.json](file://packages/api-server/drizzle/meta/0002_snapshot.json#L1-L1007)
- [packages/api-server/drizzle/meta/0003_snapshot.json](file://packages/api-server/drizzle/meta/0003_snapshot.json#L1-L1007)
- [packages/api-server/src/routes/versions.ts](file://packages/api-server/src/routes/versions.ts#L255-L312)

## 结论

本数据库设计以 Drizzle ORM 为核心，围绕会话、消息、脚本与工程版本管理构建了清晰的数据模型与索引策略。通过枚举类型与 JSONB 结构化存储，兼顾了灵活性与一致性；通过迁移与快照机制，实现了结构演进的可追踪与可回溯。

**更新** 本次重大更新实现了从混合架构到纯数据库架构的迁移，引入了模板文件系统、虚拟路径管理和弃用状态机制，为多用户项目基础设施提供了坚实的数据基础。新的架构支持系统模板的数据库化存储，消除了对物理文件系统的依赖，提升了部署的便利性和可扩展性。

结合 API 路由，上述模型有效支撑了会话生命周期管理、聊天记录持久化、脚本存储与工程版本控制等核心功能，并为未来的模板管理和多租户支持奠定了基础。
...
- [docs/MIGRATION_TO_DATABASE_ARCHITECTURE.md](file://docs/MIGRATION_TO_DATABASE_ARCHITECTURE.md#L80-L230)

### 模板文件系统架构

```mermaid
flowchart LR
A["系统模板目录"] --> B["ProjectInitializer"]
B --> C["script_files表"]
C --> D["模板文件类型: template"]
C --> E["虚拟路径: file_path"]
E --> F["_system/config/default/ai_ask_v1.md"]
E --> G["_system/config/custom/{scheme}/ai_say_v1.md"]
```

**图表来源**

- [packages/api-server/src/services/project-initializer.ts](file://packages/api-server/src/services/project-initializer.ts#L107-L169)
- [packages/api-server/src/db/schema.ts](file://packages/api-server/src/db/schema.ts#L155-L187)
