...
## 更新摘要
**变更内容**
- 新增模板引擎章节，详细介绍提示词模板管理系统
- 新增变量作用域引擎章节，说明分层变量存储和作用域解析机制
- 更新脚本执行引擎章节，反映更高效的数据库查询模式和执行状态管理
- 更新调试信息管道章节，说明改进的调试信息捕获和传递机制
- 新增变量存储系统章节，说明从简单变量到分层变量的演进
...
- **新增** 模板引擎：负责加载和管理提示词模板，支持两层变量替换（系统变量和脚本变量），提供模板缓存和验证功能。
- **新增** 变量作用域引擎：负责变量的作用域解析与优先级查找，支持全局、会话、阶段、话题四个作用域，提供变量定义管理和作用域决策。
