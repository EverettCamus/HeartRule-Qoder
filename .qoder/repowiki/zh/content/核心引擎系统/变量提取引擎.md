...
## 项目结构
变量提取引擎位于核心引擎包中，围绕“变量配置-提取器-领域模型-共享类型”形成清晰分层：
- 引擎层：变量提取器与导出入口
- 领域层：变量状态与历史管理
...
Client->>SM : "提交用户输入"
SM->>VE : "extract(userInput, history, configs)"
...
VS["VariableState"] --> ENUM["VariableScope/UpdateMode"]
...
## 结论
变量提取引擎通过“直接/正则/LLM”三通道与“类型转换+Schema校验”的组合，实现了灵活且稳健的变量抽取能力。配合会话管理器与脚本配置，可在不同阶段与话题中精确控制变量的作用域与更新模式。通过批量提取、上下文裁剪与可扩展缓存，引擎具备良好的性能与可维护性。
