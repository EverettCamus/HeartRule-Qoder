...
## 更新摘要
**变更内容**
- 新增DDD应用服务层的SessionApplicationService作为防腐层的设计理念
- 新增ISessionApplicationService接口定义和DefaultSessionApplicationService实现
- 更新架构总览以反映应用服务层的引入
- 新增应用服务层与领域模型的交互关系说明
- 更新性能考虑和故障排查指南以包含应用服务层
...
## 引言
本文件系统化梳理 HeartRule AI 咨询引擎的领域模型，围绕三大核心实体 Session（会话）、Script（脚本）、Message（消息）展开，阐明其设计理念、业务含义、生命周期与状态流转、变量管理机制，以及三者之间的关系与约束。同时结合执行引擎、应用服务层与 API 层实现，说明数据模型如何支撑完整的咨询流程，并给出扩展与自定义字段的实践建议。

**更新** 新增DDD应用服务层的设计，通过SessionApplicationService作为防腐层隔离核心引擎与API层的调用关系。

## 项目结构
- 领域模型位于核心引擎包中，采用按领域分层组织：domain 目录下分别定义 Session、Script、Message、Variable 等领域模型类与接口。
- 应用服务层提供标准接口定义，作为防腐层隔离核心引擎与外部系统的调用关系。
...
App_Service["SessionApplicationService 应用服务"]
...
App_Service --> Domain_Session
App_Service --> Domain_Script
App_Service --> Domain_Message
App_Service --> Domain_Variable
Engine_Executor --> Route_Sessions
Engine_Executor --> Route_Scripts
Session_Manager --> App_Service
...
- [packages/core-engine/src/application/session-application-service.ts](file://packages/core-engine/src/application/session-application-service.ts#L138-L160)
...
- [packages/core-engine/src/application/session-application-service.ts](file://packages/core-engine/src/application/session-application-service.ts#L1-L514)
...
- **SessionApplicationService（会话应用服务）**
  - **新增** 作为防腐层，定义会话执行的标准接口形态，隔离核心引擎的内部实现细节与 API 层的调用关系。
  - 提供 initializeSession 和 processUserInput 两个核心接口，封装会话初始化与用户输入处理的完整流程。
...
## 架构总览
会话从创建开始，绑定脚本后进入执行循环：应用服务层作为防腐层隔离核心引擎与API层，执行器解析脚本结构，按阶段/话题/动作顺序驱动 Action 执行，期间维护执行状态、变量集合与对话历史，并在需要用户输入时返回"等待输入"状态。API 层负责会话与脚本的创建、查询与消息交互，数据库持久化会话、消息与脚本。

**更新** 新增应用服务层作为防腐层的设计，明确API层与核心引擎的边界。

```mermaid
sequenceDiagram
participant Client as "客户端"
participant API as "会话路由"
participant AppSvc as "会话应用服务"
participant Manager as "会话管理器/执行器"
participant DB as "数据库"
participant Script as "脚本内容"
Client->>API : "POST /api/sessions"
API->>AppSvc : "initializeSession(request)"
AppSvc->>Manager : "initializeSession(sessionId)"
Manager->>Script : "读取并解析脚本"
Manager->>Manager : "执行首轮 Action如 ai_say"
Manager-->>AppSvc : "返回执行状态与结果"
AppSvc-->>API : "封装为SessionExecutionResponse"
API-->>Client : "会话创建成功含初始 AI 消息"
Client->>API : "POST /api/sessions/{id}/messages"
API->>AppSvc : "processUserInput(request)"
AppSvc->>Manager : "processUserInput(sessionId, content)"
Manager->>Manager : "继续执行 Action可能等待用户输入"
Manager-->>AppSvc : "返回 AI 响应/等待输入/错误"
AppSvc-->>API : "封装为SessionExecutionResponse"
...
- [packages/core-engine/src/application/session-application-service.ts](file://packages/core-engine/src/application/session-application-service.ts#L182-L233)
...
### SessionApplicationService（会话应用服务）接口设计
**新增** 作为DDD应用服务层的核心组件，提供标准接口定义和防腐层隔离。

- **接口职责**
  - 定义会话执行的标准接口形态
  - 隔离核心引擎的内部实现细节与 API 层的调用关系
  - 作为会话执行业务能力与外部系统的防腐层（Anti-Corruption Layer）
...
- **核心接口**
  - `initializeSession(request)`: 初始化会话并返回第一条 AI 消息
  - `processUserInput(request)`: 处理用户输入并推进会话执行
...
+constructor(scriptExecutor?)
+initializeSession(request) Promise~SessionExecutionResponse~
+processUserInput(request) Promise~SessionExecutionResponse~
-private createInitialExecutionState() any
-private restoreExecutionState() any
-private buildResponse() SessionExecutionResponse
-private buildErrorResponse() SessionExecutionResponse
}
...
**图表来源**
- [packages/core-engine/src/application/session-application-service.ts](file://packages/core-engine/src/application/session-application-service.ts#L138-L160)
- [packages/core-engine/src/application/session-application-service.ts](file://packages/core-engine/src/application/session-application-service.ts#L169-L175)

### SessionExecutionResponse（会话执行响应）
**新增** 应用服务层的标准输出格式，封装完整的执行结果。

- **核心字段**
  - `aiMessage`: AI 生成的消息内容
  - `executionStatus`: 执行状态（running/waiting_input/completed/error）
  - `position`: 扩展的执行位置信息，包含 phaseIndex/topicIndex/actionIndex 及可选 ID 字段
  - `variables`: 会话级变量（扁平结构）
  - `variableStore`: 分层变量存储（用于调试与内部状态同步）
  - `debugInfo`: LLM 调试信息
  - `error`: 错误信息（如果执行失败）

### 请求参数设计
**新增** 应用服务层的输入参数结构，确保业务数据与基础设施解耦。

- **InitializeSessionRequest**
  - `sessionId`: 会话 ID
  - `scriptContent`: 脚本内容（JSON 字符串或解析后的对象）
  - `globalVariables`: 全局变量
  - `sessionVariables`: 会话级变量（用于恢复会话）
...
- **ProcessUserInputRequest**
  ...
  - `scriptContent`: 脚本内容
  - `globalVariables`: 全局变量
  - `currentExecutionState`: 当前执行状态（用于恢复）
...
- 应用服务层与领域模型
  - 应用服务层作为防腐层，定义标准接口隔离核心引擎与外部系统调用关系。
...
- API 层与数据库
  ...
  - 会话管理器作为应用服务层的具体实现，负责与数据库交互和状态持久化。
...
Domain_Session --> App_Service["SessionApplicationService"]
Domain_Script --> App_Service
Domain_Message --> App_Service
Domain_Variable --> App_Service
App_Service --> Engine_Executor["脚本执行器"]
App_Service --> Route_Sessions["会话路由"]
...
- **应用服务层性能监控**：应用服务层包含详细的性能日志记录，对超过阈值的执行进行警告（初始化 > 1000ms，用户输入 > 2000ms）。
...
- **应用服务层问题排查**
  - 检查应用服务层的日志输出，确认请求参数是否正确传递。
  - 验证 DefaultSessionApplicationService 的执行状态和响应格式。
  - 确认 ScriptExecutor 的初始化和执行过程是否正常。
...
- [packages/core-engine/src/application/session-application-service.ts](file://packages/core-engine/src/application/session-application-service.ts#L224-L226)

## 结论
HeartRule 的领域模型以 Session、Script、Message 为核心，配合 Variable 的灵活更新与历史回滚能力，形成完整的咨询流程数据骨架。**新增的DDD应用服务层**通过 SessionApplicationService 作为防腐层，明确了 API 层与核心引擎的边界，实现了更好的解耦和可维护性。执行引擎将脚本结构转化为可编排的动作流，API 层提供稳定的交互入口，数据库 Schema 支撑复杂结构的持久化。该设计既保证了业务语义清晰，又具备良好的扩展性与可维护性。

**更新** 新增DDD应用服务层的设计理念和防腐层的作用，提升了系统的架构质量和可维护性。
...
- **应用服务层**
  - **新增** 接口设计遵循DDD原则，输入参数只包含业务标识与数据，输出结果包含完整的执行结果与状态。
...
- **应用服务层初始化**：[packages/core-engine/src/application/session-application-service.ts](file://packages/core-engine/src/application/session-application-service.ts#L182-L233)
- **应用服务层用户输入处理**：[packages/core-engine/src/application/session-application-service.ts](file://packages/core-engine/src/application/session-application-service.ts#L240-L294)
...
- **新增应用服务层接口**
  - **新增** 在 ISessionApplicationService 接口中定义新的业务方法，确保输入参数只包含业务标识与数据。
  - **新增** 实现类中提供相应的业务逻辑，遵循应用服务层的设计原则。
