---
document_id: openspec-specs-research-technical-ai-say-prompt-template-exploration-md
authority: research
status: exploratory
version: 0.1.0
last_updated: 2025-01-14
source: design-thinking
path: specs/research/technical/ai-say-prompt-template-exploration.md
tags: [research, exploratory, technical, ai-implementation, prompt-engineering, ai-say]
search_priority: medium
language: zh
category: research/technical
research_status: thinking-in-progress
purpose: Exploratory thinking about ai_say prompt template design patterns and three-layer architecture implementation
related_documents: ['specs/domain/tactical/ai-say-prompt-template-specification.md']
---
...
**任务 2：拆解为基线步骤 base_steps** 3. 将整个话题拆解为 3-7 个有清晰顺序的步骤（字符串数组），输出为 base_steps：
...
**任务 3：初始化策略步骤 strategy_steps** 4. 在 base_steps 的基础上，初始化可执行的 strategy_steps：
...
**任务 4：生成初始沟通策略 strategy_prompt** 5. 结合用户画像与 subtype_hint，生成一段给主线 A 使用的沟通策略提示（strategy_prompt）：
...
**任务 5：给出初始化理由（initialization_reasoning）** 6. 用 2-4 句自然语言解释你为什么这样拆解步骤、为什么这样设置初始策略。
...
**任务 2：评估尝试意愿与重要顾虑** 3. 判断用户是否明确表达“愿意尝试”或等价表述，输出：expressed_willingness (true|false) 4. 判断是否仍有尚未回应的重要顾虑（例如安全担心、时间精力、价值冲突等），输出：has_major_concerns (true|false) 5. 如有，归纳 1-3 条最关键的顾虑，输出：major_concerns (数组，元素为自然语言短句)

**任务 3：说服进度总结与步骤调整判断（阶段 1+）** 6. 总结当前说服阶段的进度（用户目前接受到哪一步、还有哪些关键点未覆盖），输出：progress_summary 7. 结合 `base_steps` 与 `{%last_strategy_steps%}`，判断是否需要调整当前执行计划（例如：简化步骤、增加铺垫、重新排序），输出：step_adjustment_recommendation：
...
**任务 4：沟通策略生成** 9. 基于用户画像、上述评估结果与更新后的 strategy_steps，生成下一轮说服/邀请对话的沟通策略，输出：strategy_prompt
