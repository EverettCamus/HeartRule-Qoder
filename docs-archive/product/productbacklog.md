---
document_id: 'docs-archive-product-productbacklog'
authority: 'historical'
status: 'archived'
archived_date: '2026-03-13'
source: 'docs'
path: 'docs/product/productbacklog.md'
migrated_to: '' # 如果已迁移到openspec，填写openspec路径
tags: ['historical', 'reference', 'archived', 'product']
...
**归档日期**: 2026-03-13
**原始路径**: docs/product/productbacklog.md

---

---

document_id: docs-product-productbacklog-md
authority: historical
status: archived
version: 0.9.0
...
source: docs
path: product/productbacklog.md
...
## Epic 0: 多用户脚本工程与数据库基础设施(脚本仓库层)

**Epic目标**: 为多用户、多工程场景提供统一的数据库脚本仓库与隔离机制,奠定四层咨询架构和调试能力的基础运行环境。

**优先级**: P0 - 最高(平台基础设施,必须优先)

### Story 0.1: 脚本工程数据库模型与目录语义统一
...
- 明确`projects`、`script_files`、`project_versions`三张表在"脚本仓库"中的职责边界
- 规范`script_files.fileType`与脚本层级/文件职责的映射关系(全局/角色/技能/Session脚本/模板等)
- 统一`filePath`虚拟路径约定,覆盖`_system/config/default`、`_system/config/custom/{scheme}`等模板路径
- 明确版本化策略: Session/Phase/Topic/Action及模板文件在`project_versions.versionFiles`中的快照结构
...
### Story 0.2: 多租户与项目隔离机制(面向平台服务)

**Story描述**: 作为平台服务,我需要在数据库层面实现多用户/多项目的隔离机制,确保不同咨询工程师和不同项目之间的脚本工程与会话数据不会相互干扰,为后续SaaS化铺路。

**详细需求**:

- 明确`projects.author`/`tags`/`metadata`中用于标识租户/所有者/协作者的字段约定
- 在API层引入"当前租户/当前用户"上下文,所有项目/脚本/会话查询均带上租户过滤条件
- 为数据库设计预留Row-Level Security(RLS)的策略空间(如按tenant_id/project_id隔离),即使短期内不启用RLS,也保证未来可平滑切换
- 会话(Session)通过`session.metadata.projectId`、`sessions.versionId`与具体项目/版本强绑定,防止脚本切换导致状态残留
...
### Story 0.3: 会话-项目-版本绑定与脚本快照机制

**Story描述**: 作为系统,我需要在创建和执行会话时,将会话与具体项目及发布版本绑定,并在需要时使用版本快照,确保同一会话生命周期内脚本定义稳定不变,避免调试过程受到线上脚本变更影响。

**详细需求**:

- Session创建流程中,从脚本或工程标签中解析出`projectId`并写入`sessions.metadata.projectId`
- 支持在发布版本时为项目生成`project_versions.versionFiles`快照,包含所有脚本文件与模板
- 会话可选择绑定到某个版本(`sessions.versionId`),执行时优先从版本快照加载脚本,否则从当前工程文件加载
- 调试/回放场景中,可基于versionSnapshot复现当时的脚本状态
...
### Story 0.4: TemplateProvider与PromptTemplateManager数据库优先模式

**Story描述**: 作为核心引擎,我希望提示词模板系统默认使用数据库中的模板文件(script_files, fileType='template'),仅在本地开发或兼容旧工程时才回退到workspace文件系统,以简化部署和多用户脚本管理。

**详细需求**:

- 在core-engine中,为PromptTemplateManager/TemplateResolver配置TemplateProvider接口的数据库实现(DatabaseTemplateProvider)为默认路径
- API层(SessionManager等)在构造ScriptExecutor时,为其注入projectId与TemplateProvider,使模板解析直接走数据库
- workspace仍作为运行时上下文(调试/临时文件)存在,但不再作为模板文件的唯一来源
- 为旧工程保留受控的磁盘回退逻辑,并在文档中标注为"兼容模式",避免新项目继续依赖

**验收标准**:

- [x] 默认配置下,模板加载全部通过数据库脚本工程完成,不依赖磁盘模板文件
- [x] 至少一个完整会话流程(E2E测试)在无workspace模板目录情况下可以顺利执行
- [x] 对旧工程启用兼容模式时,模板加载行为与当前版本一致

**实现状态**: ✅ 已完成

- DatabaseTemplateProvider已实现(database-template-provider.ts)
- TemplateResolver支持两种模式(template-resolver.ts)
- TemplateManager支持DatabaseTemplateProvider注入(template-manager.ts)
- SessionManager已注入DatabaseTemplateProvider作为默认provider
- 监控模板(ai_ask_monitor_v1.md、ai_say_monitor_v1.md)支持数据库加载
...
- [x] ai_ask/ai_say/ai_think都有默认模板
- [x] 默认模板包含角色定义、输出格式、安全规范
- [x] 脚本只写業务提示词即可运行
- [x] 支持两层模板方案机制(default + custom),可通过 session 节点的 `template_scheme` 字段指定使用的方案
- [~] 单元测试覆盖模板渲染逻辑
...
- 监控模板(ai_ask_monitor_v1.md、ai_say_monitor_v1.md)已创建,支持两层方案
- 模板包含完整的metrics输出结构定义和"Topic层策略建议"占位符
- 部分单元测试已覆盖模板渲染逻辑,待完善全面测试

**优先级**: P0 - 高  
**预估工作量**: 5 Story Points  
**依赖关系**: 依赖Story 0.4的数据库模板加载基础
...
- [x] ActionResult新增metrics字段
- [x] ai_ask返回information_completeness和reply_relevance
- [x] ai_say返回user_engagement
- [x] 指标值合理(LLM评估依据)
- [x] executionState.metadata可查询历史指标
- [x] Action层提供progress_suggestion建议

**实现状态**: ✅ 已完成

- ActionResult完整实现metrics字段和progress_suggestion
- ai_ask返回完整metrics(information_completeness、user_engagement、emotional_intensity、reply_relevance)
- ai_say返回metrics(user_engagement、emotional_intensity、understanding_level)
- ScriptExecutor存储metrics历史到actionMetricsHistory
- 监控处理器(AiAskMonitorHandler、AiSayMonitorHandler)完整实现
- 监控反馈闭环机制已实现,影响Action主线程提示词
- TopicActionOrchestrator扩展点接口已定义(预留未来实现)
- 完成日期: 2026-02-13
...
- TopicActionOrchestrator接口完整定义(orchestration/topic-action-orchestrator.ts)
- OrchestrationPlan数据结构完整定义
